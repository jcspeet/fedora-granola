"""
Chat with meeting context via Claude, OpenAI, or local Ollama.
"""
...
class Chatter:
    ...
    def _run(self, messages: list[dict], system_prompt: str):
        ...
        if provider == "openai":
            ...
            self._run_ollama(messages, system_prompt)
        else:
    ...
    def _run_anthropic(self, messages: list[dict], system_prompt: str):
        if not config.ANTHROPIC_API_KEY:
            if self.on_error:
                self.on_error("ANTHROPIC_API_KEY is not set. Open Settings.")
            return
    ...
    def _run_openai(self, messages: list[dict], system_prompt: str):
        if not config.OPENAI_API_KEY:
            if self.on_error:
                self.on_error("OPENAI_API_KEY is not set. Open Settings.")
            return
    ...
    def _run_ollama(self, messages: list[dict], system_prompt: str):
        try:
